...
# print('result', result)
# for i in range(0, len(raw_seeds), 2):
#     seeds = [x for x in range(raw_seeds[i], raw_seeds[i]+raw_seeds[i+1])]
#     for seed in seeds:
#         print(seed)
#         location = get_location(seed, puzzle)
#         if locations == 0:
...
raw_seeds = np.array([int(seed) for seed in get_seeds(puzzle)])  # Your raw_seeds array
...
for i in range(0, len(raw_seeds), 2):
    ...
    seed_locations = np.array([get_location(seed, puzzle) for seed in seeds])
    min_location = np.min(seed_locations)
    
    if locations == 0:
        locations = min_location
    else:
        locations = min(locations, min_location)
